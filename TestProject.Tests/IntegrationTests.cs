...
namespace TestProject.Tests
{
    public class IntegrationTests
    {
        ...
        private async Task SeedData()
        {
            ...
            var createForm1 = GenerateCreateForm("Ticket Title 2", "In reasonable compliment favourable is connection dispatched in terminated. Do esteem object we called father excuse remove. So dear real on like more it. Laughing for two families addition expenses surprise the. If sincerity he to curiosity arranging. Learn taken terms be as. Sbookcely mrs produced too removing new old. ", "William F.", DateTime.Parse("03.05.2020"));
            ...
            var createForm3 = GenerateCreateForm("Ticket Title 4", "Improved own provided blessing may peculiar domestic. Sight house has sex never. No visited raising gravity outward subject my cottage mr be. Hold do at tore in park feet near my case. Invitation at understood occasional sentiments insipidity inhabiting in. Off melancholy alteration principles old. Is do speedily kindness properly oh. Respect article painted cottage he is offices parlors. ", "John D.", DateTime.Parse("06.11.2019"));
        ...
        private TicketForm GenerateCreateForm(string title, string body, string authorName, DateTime publishedDate)
        {
        ...
        public async Task Test1()
        {
            ...
            var books = JsonConvert.DeserializeObject<IEnumerable<Ticket>>(response0.Content.ReadAsStringAsync().Result);
        ...
        public async Task Test2()
        {
            ...
            var book = JsonConvert.DeserializeObject<Ticket>(response0.Content.ReadAsStringAsync().Result);
            book.Title.Should().Be("Ticket Title 1");
            book.Description.Should().Be("Patrick B.");
        ...
        public async Task Test3()
        {
            ...
            var filteredbooks = JsonConvert.DeserializeObject<IEnumerable<Ticket>>(response1.Content.ReadAsStringAsync().Result).ToArray();
            filteredbooks.Length.Should().Be(3);
            filteredbooks.Where(x => x.Description == "John D.").ToArray().Length.Should().Be(1);
            filteredbooks.Where(x => x.Description == "Patrick B.").ToArray().Length.Should().Be(2);
        ...
        public async Task Test5()
        {
            ...
            var book = JsonConvert.DeserializeObject<Ticket>(response1.Content.ReadAsStringAsync().Result);
            response1.StatusCode.Should().BeEquivalentTo(StatusCodes.Status200OK);
